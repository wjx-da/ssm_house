var Rent_manage_tool = null; 
$(function () { 
	initPriceRangeManageTool(); //建立PriceRange管理对象
	Rent_manage_tool.init(); //如果需要通过下拉框查询，首先初始化下拉框的值
	$("#Rent_manage").datagrid({
		url : 'Rent/list',
		fit : true,
		fitColumns : true,
		striped : true,
...
function initPriceRangeManageTool() {
	Rent_manage_tool = {
		...
		reload : function () {
			$("#Rent_manage").datagrid("reload");
		},
		redo : function () {
			$("#Rent_manage").datagrid("unselectAll");
		},
		search: function() {
			$("#Rent_manage").datagrid("load");
		},
		...
		remove : function () {
			var rows = $("#Rent_manage").datagrid("getSelections");
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						$.ajax({
							...
							beforeSend : function () {
								$("#Rent_manage").datagrid("loading");
							},
							success : function (data) {
								if (data.success) {
									$("#Rent_manage").datagrid("loaded");
									$("#Rent_manage").datagrid("load");
									$("#Rent_manage").datagrid("unselectAll");
									$.messager.show({
										title : "提示",
